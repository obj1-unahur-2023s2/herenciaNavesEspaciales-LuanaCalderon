...
class NaveBaliza inherits Naves {
	...
	override  method pocaActividad() {return not cambioColorBaliza}
}
...
class Hospital inherits NavePasajeros {
...
class NavesDeCombate inherits Naves {
	...
	method esEscueta1() = not mensajes.any({m=> m.size() >30})
	...
	override method avisar(){ return self.emitirMensaje("Amenaza recibida") }
	
	override  method pocaActividad() { return self.esEscueta()}

}


class Sigilosa inherits NavesDeCombate{
	
	override method estaTranquila() {  return super() and visible}
